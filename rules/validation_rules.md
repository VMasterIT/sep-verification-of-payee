...
5. [Валідація Коду ID НБУ](#валідація-коду-id-нбу-nbu-id)
6. [Валідація BIC (опціонально)](#валідація-bic-опціонально)
7. [Валідація UUID](#валідація-uuid)
8. [Валідація timestamp](#валідація-timestamp)
9. [Валідація request payload](#валідація-request-payload)
10. [Санітизація даних](#санітизація-даних)
...
## 5. Валідація Коду ID НБУ (NBU ID)

### 5.1 Формат Коду ID НБУ

**VR-NBUID-01: Довжина**
```
Вимога: 6 цифр
Приклад: 305299 (ПриватБанк), 300001 (NBU Bank)
```
...
Regex: ^[0-9]{6}$
Тільки цифри: 0-9
Довжина: рівно 6 символів
```

**VR-NBUID-03: Діапазон**
```
Мінімум: 300001
Максимум: 399999
Діапазон: коди банків у реєстрі НБУ
```

**Приклад:**
```python
def validate_nbu_id(nbu_id):
    """
    Валідація Коду ID НБУ
    """
    # 1. Довжина
    if len(nbu_id) != 6:
        return {
            'valid': False,
            'error': f'Invalid NBU ID length: {len(nbu_id)} (expected 6)'
        }

    # 2. Формат (тільки цифри)
    if not re.match(r'^[0-9]{6}$', nbu_id):
        return {
            'valid': False,
            'error': 'Invalid NBU ID format (must be 6 digits)'
        }

    # 3. Діапазон
    nbu_id_int = int(nbu_id)
    if not (300001 <= nbu_id_int <= 399999):
        return {
            'valid': False,
            'error': f'NBU ID {nbu_id} is out of valid range (300001-399999)'
        }
...
**Примітка:** Код ID НБУ є основним ідентифікатором банку в Україні. Всі банки-учасники СЕП мають унікальний 6-значний код.

---

## 6. Валідація BIC (опціонально)

**Примітка:** BIC (Bank Identifier Code, ISO 9362) є опціональним полем для міжнародної інтеграції. В Україні основний ідентифікатор - Код ID НБУ.

### 6.1 Формат BIC (ISO 9362)
