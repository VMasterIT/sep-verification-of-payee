...
### 1.5 Ідентифікація учасників: Код ID НБУ vs BIC

#### Основний ідентифікатор: nbuId (Код ID НБУ)

В VoP системі СЕП НБУ **основним ідентифікатором** учасника є `nbuId` - Код ID НБУ (ідентифікатор учасника в Системі електронних платежів НБУ).

**Характеристики Коду ID НБУ:**
- **Формат:** 6 цифр (наприклад: `300001`, `300465`, `305299`, `322001`)
- **Область використання:** Внутрішні українські перекази через СЕП НБУ
- **Обов'язковість:** **ОБОВ'ЯЗКОВИЙ** в усіх VoP запитах та відповідях
- **Унікальність:** Кожен учасник СЕП НБУ має унікальний Код ID НБУ
- **Приклади реальних кодів:**
...
**Примітка:** Код ID НБУ - це сучасна назва ідентифікатора. Застаріла назва "МФО код" більше не використовується в офіційній документації НБУ.
...
#### Опціональний ідентифікатор: BIC (Bank Identifier Code)

Поле `bic` є **ОПЦІОНАЛЬНИМ** в поточній версії VoP системи.

**Характеристики BIC:**
- **Формат:** 8 або 11 символів (ISO 9362)
  - 8 символів: `PRYBUA2X` (базовий BIC)
  - 11 символів: `PRYBUA2XXXX` (BIC з branch code)
- **Область використання:** Міжнародні перекази через SWIFT
- **Обов'язковість:** **ОПЦІОНАЛЬНИЙ** для внутрішніх переказів СЕП НБУ
- **Майбутнє використання:** Буде використовуватись для транскордонних переказів
- **Приклади:**
  - `NBUBUBU1XXX` - Національний банк України
  - `PRYBUA2XXXX` - ПриватБанк
  - `PBANUA2X` - ПриватБанк (скорочена форма)
  - `MONOUA2X` - Monobank

**Коли використовувати BIC:**

✅ **Рекомендовано передавати BIC у таких випадках:**
- Банк має намір підтримувати транскордонні перекази в майбутньому
- Для сумісності з міжнародними стандартами
- Якщо BIC вже наявний в системі банку

❌ **Можна не передавати BIC:**
- Для внутрішніх українських переказів (достатньо Коду ID НБУ)
- Якщо BIC ще не отриманий від SWIFT
- Для спрощення інтеграції на початковому етапі

**Приклад з обома ідентифікаторами:**
...
**Приклад тільки з Кодом ID НБУ (валідний):**
...
| Параметр | Код ID НБУ (`nbuId`) | BIC (`bic`) |
|----------|----------------------|-------------|
| **Формат** | 6 цифр | 8-11 символів (літери + цифри) |
| **Приклад** | `322001` | `MONOUA2X` |
| **Стандарт** | Національний (НБУ) | Міжнародний (ISO 9362 / SWIFT) |
| **Використання** | СЕП НБУ (внутрішні перекази) | SWIFT (міжнародні перекази) |
| **Обов'язковість в VoP** | ✅ **Обов'язковий** | ⭕ Опціональний |
| **Область дії** | Україна | Глобальна |
| **Майбутнє в VoP** | Продовжить використання | Для транскордонних переказів |

#### Відображення в Directory Service

Directory Service зберігає обидва ідентифікатори для кожного учасника:

```json
{
  "participantId": "bank-privatbank",
  "nbuId": "305299",
  "bic": "PRYBUA2XXXX",
  "name": "АТ КБ \"ПРИВАТБАНК\"",
  "status": "ACTIVE",
  "capabilities": ["vop_requester", "vop_responder"]
}
```

Маршрутизація VoP запитів **завжди відбувається по Коду ID НБУ** (`nbuId`), навіть якщо BIC також присутній в запиті.

#### Рекомендації для розробників

1. **Завжди передавайте `nbuId`** - він обов'язковий для всіх операцій
2. **Передавайте `bic` якщо він доступний** - для майбутньої сумісності
3. **Не покладайтеся на BIC** для маршрутизації - використовується тільки `nbuId`
4. **Зберігайте обидва коди** в своїй системі для повної сумісності
5. **Валідуйте формат:**
   - `nbuId`: regex `^\d{6}$`
...
| `requester.bic` | string | Ні | BIC банку-відправника (опціональний, для майбутніх транскордонних переказів) | ISO 9362, 8-11 символів |
| `requester.nbuId` | string | Так | Код ID НБУ банку-відправника (основний ідентифікатор в СЕП НБУ) | 6 цифр |
| `payee.iban` | string | Так | IBAN отримувача | 29 символів, UA + 27 цифр |
| `payee.name` | string | Так | Ім'я отримувача | 1-140 символів |
| `payee.identificationType` | string | Ні | Тип ідентифікатора | EDRPOU, RNOKPP, PASSPORT |
...
| `responder.bic` | string | Ні | BIC банку-отримувача (опціональний, для майбутніх транскордонних переказів) |
| `responder.nbuId` | string | Так | Код ID НБУ банку-отримувача (основний ідентифікатор в СЕП НБУ) |
