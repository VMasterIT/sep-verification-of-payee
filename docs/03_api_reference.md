...
### 1.5 Ідентифікація учасників: МФО vs BIC

#### Основний ідентифікатор: nbuId (МФО код)

В VoP системі СЕП НБУ **основним ідентифікатором** учасника є `nbuId` - МФО код (Міжбанківський ідентифікаційний код).
...
- **Унікальність:** Кожен учасник СЕП НБУ має унікальний МФО код
- **Приклади:**
...
| Параметр | МФО (`nbuId`) | BIC (`bic`) |
|----------|---------------|-------------|
| **Формат** | 6 цифр | 8-11 символів (літери + цифри) |
| **Приклад** | `300023` | `PRYBUA2XXXX` |
...
Маршрутизація VoP запитів **завжди відбувається по МФО коду** (`nbuId`), навіть якщо BIC також присутній в запиті.
...
| `requester.nbuId` | string | Так | МФО код банку-відправника (основний ідентифікатор в СЕП НБУ) | 6 цифр |
...
| `responder.nbuId` | string | Так | МФО код банку-отримувача (основний ідентифікатор в СЕП НБУ) |
