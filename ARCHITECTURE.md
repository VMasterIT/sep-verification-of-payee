...
**Примітка:** Поле `bic` є опціональним. Обов'язковим є тільки `nbuId` (Код ID НБУ). BIC використовується для майбутніх транскордонних переказів.
...
**Примітка:** Поле `bic` є опціональним. Обов'язковим є тільки `nbuId` (Код ID НБУ).
...

{
  ...
  "responder": {
    ...
    "nbuId": "305299"      // Обов'язково (Код ID НБУ)
  },
  "result": {
    "matchStatus": "MATCH",
    "matchScore": 100,
